...
export default function NewDepartmentPage() {
  ...
  const { control, handleSubmit, watch, setValue, formState: { errors, isSubmitting } } = useForm<CreateDepartmentDto>({
  ...
  const onSubmit = async (data: CreateDepartmentDto) => {
    try {
      ...
      toast.showSuccess('Reparto creato con successo');
      router.push('/departments');
    } catch (error) {
      console.error(error);
      toast.showError('Errore durante la creazione del reparto');
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Nuovo Reparto</h1>
        <button
          onClick={() => router.back()}
