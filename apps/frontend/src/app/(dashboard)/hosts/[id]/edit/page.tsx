...
export default function EditHostPage() {
  ...
  useEffect(() => {
    ...
    hostsApi.getOne(id).then((res) => {
      ...
      toast.showError('Referente non trovato');
      router.push('/hosts');
    });
  }, [id]);
  ...
  const onSubmit = async (data: yup.InferType<typeof schema>) => {
    try {
      ...
      toast.showSuccess('Referente aggiornato');
      router.push('/hosts');
    } catch {
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Modifica Referente</h1>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <Controller name="firstName" control={control} render={({ field }) => (
                <input {...field} type="text" className={`input ${errors.firstName ? 'border-red-500' : ''}`} />
              )} />
            ...
            <div>
              ...
              <Controller name="lastName" control={control} render={({ field }) => (
                <input {...field} type="text" className={`input ${errors.lastName ? 'border-red-500' : ''}`} />
              )} />
            ...
            <div>
              ...
              <Controller name="phone" control={control} render={({ field }) => (
                <input {...field} type="tel" className="input" />
              )} />
            </div>
          ...
          <div className="flex justify-end gap-3 mt-8 pt-6 border-t border-gray-100">
            ...
            <button type="submit" disabled={isSubmitting} className="btn btn-primary">
              {isSubmitting ? (
                <><div className="animate-spin w-5 h-5 border-2 border-white border-t-transparent rounded-full"></div>Salvataggio...</>
              ) : (
