...
export default function EditVisitorPage() {
  ...
  const onSubmit = async (data: yup.InferType<typeof schema>) => {
    try {
      ...
      console.error(error);
      toast.showError('Errore durante l\'aggiornamento del visitatore');
    }
  };
    ...
    >
      ...
      {visitor && (
        <nav className="flex items-center gap-2 text-sm mb-4">
          ...
          <ChevronRightIcon />
          <Link href={`/visitors/${id}`} className="text-gray-500 hover:text-gray-700 transition-colors">
      ...
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Modifica Visitatore</h1>
        <button
          onClick={() => router.back()}
            ...
            <div>
              ...
              <label className="flex items-center gap-2 px-4 py-3 border-2 border-dashed border-gray-300 rounded-xl cursor-pointer hover:border-gray-400 hover:bg-gray-50 transition-colors">
                ...
                <span className="text-sm text-gray-600">
                  {documentFile ? documentFile.name : 'Carica Nuovo Documento'}
                </span>
                <input
                  type="file"
                  className="hidden"
